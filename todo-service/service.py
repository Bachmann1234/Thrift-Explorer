...
def _get_db():
    ...
    with db:
        db.execute("create table if not exists task (id integer primary key, description varchar, duedate varchar);")
    return db
...
class Dispatcher(object):
    ...
    def createTask(self, description, due_date):
        with _get_db() as db:
            cursor = db.cursor()
            cursor.execute("insert into task (description, duedate) values(?, ?)", (description, due_date))
